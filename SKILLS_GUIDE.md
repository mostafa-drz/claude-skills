...
| `/screenshots-memory` | Screenshots → a local, queryable memory. Kind-aware extraction (course/chat/product/ui, extensible), topic clusters with kind as a filter facet, single-file HTML per cluster, and a correction loop that teaches the extractor your apps. Store is local git with no remote; originals move only after the commit lands. | Yes (moves files, writes store, commits) | No |
