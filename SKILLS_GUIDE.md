...
| `/project-updates` | Drafts Linear project status updates (Done / In Progress / Next / Blockers) per project, gathering context from Linear + git + GitHub PRs + Slack + Notion + Gmail + Calendar. All site-specific values (repos, project list, Linear org slug) configured via preferences.md. Never auto-posts. Learns phrasing and per-project rules from edits over time. | Yes (drafts; writes to own skill dir) | Linear, Notion, Slack, Gmail, Calendar |
| `/release-notes` | Generates brief, truth-based release notes for a release PR by listing each squash-merged PR + summary + linked tracker tickets. Tracker-agnostic via a configurable `ticket-url-template` with `{TICKET}` placeholder — works with Linear, Jira, GitHub Issues, Asana, ClickUp, Shortcut, Plane, etc. | Yes (edits release PR via gh) | None (tracker-agnostic) |
| `/next-steps` | Generates a stakeholder-aligned next-steps checklist (PM/design/eng/leadership owners) from the current conversation + optional Linear ticket / PR / Slack thread / Notion doc. Pasteable into Slack/Notion/Linear. | No (drafts only) | Linear (optional) |
