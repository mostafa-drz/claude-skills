...
| <img src="code/screenshots-memory/icon.svg" width="22" height="22" alt="/screenshots-memory icon" valign="middle"> &nbsp; [`/screenshots-memory`](code/screenshots-memory/SKILL.md) | Turns scattered screenshots into a queryable, git-versioned memory. Syncs captures from ~/Desktop or any folder you point at, reads each one with Claude vision (no OCR key, nothing uploaded), classifies it by kind — course, chat, product, UI, or kinds you define — and extracts kind-specific fields into confidence-scored Markdown with full provenance. Clusters by topic, renders single-file HTML per cluster, answers plain-language questions, and learns your apps from every correction. The store is local git with no remote; originals move only after the commit lands; sensitive captures are asked about before anything is written down. |
